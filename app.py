...
# --- SIDEBAR : BANKROLL ---
st.sidebar.title("💰 Sleazey's Bankroll")
...
with tab1:
    ...
    st.markdown(f"**{today}** | 15 Powerful Engines Active | 100+ Data Points per Game")
    st.markdown("---")
    
    st.subheader("🔥 Top 5 Official Plays (Meilleures Opportunités du Jour)")
    
    # Simulation dynamique des Top Plays basée sur les matchs réels disponibles
    if games_data and len(games_data) > 0:
        # On simule un classement des meilleures opportunités extraites des moteurs
        top_plays_examples = [
            {"tier": "TIER 1", "match": f"{games_data[0]['away_team']} @ {games_data[0]['home_team']}", "side": f"{games_data[0]['home_team']} ML", "odds": "1.85", "units": "3 UNITS", "conf": "82%"},
            {"tier": "TIER 1", "match": f"{games_data[min(1, len(games_data)-1)]['away_team']} @ {games_data[min(1, len(games_data)-1)]['home_team']}", "side": "Under 7.5 Runs", "odds": "1.90", "units": "2.5 UNITS", "conf": "80%"},
            {"tier": "TIER 2", "match": f"{games_data[min(2, len(games_data)-1)]['away_team']} @ {games_data[min(2, len(games_data)-1)]['home_team']}", "side": "Run Line -1.5", "odds": "2.10", "units": "2 UNITS", "conf": "76%"}
        ]
            ...
            <div class='tier1-card'>
                <table width="100%">
                    <tr>
                        ...
                        <td width="35%"><b>{play['match']}</b><br><span style="color:#8b949e;">Analyse globale validée par les moteurs</span></td>
    ...
    with c1:
        st.markdown("### 🎯 Best Totals")
        st.markdown("- **Under 7.5 Runs** (Match 1) 📉\n- **Over 8.5 Runs** (Match 2) 📈")
    with c2:
        st.markdown("### ⚾ First Five (F5)")
        st.markdown("- **Giants F5 ML** (Confiance 81%)\n- **Hawks F5 -0.5** (Confiance 78%)")
    with c3:
        st.markdown("### 🚀 Underdog Value")
        st.markdown("- **Swallows ML (+115)**\n*Small Value Lean if price improves*")

# ==========================================
# ONGLET 2 : ANALYSE MATCH DEEP-DIVE
# ==========================================
with tab2:
    st.title("🔬 Full Engine Run - Deep-Dive Match Analysis")
    ...
    if games_data and len(games_data) > 0:
        ...
        # En-tête du match style Sleazey's Edge
        st.markdown(f"""
        <div style="background-color:#161b22; padding:20px; border-radius:10px; border:1px solid #30363d; margin-bottom:20px;">
            ...
            <p style="text-align:center; color:#8b949e;">STADIUM: Tokyo Dome (Dome Environment) | TEMPERATURE: 22°C (Indoor)</p>
        </div>
        """, unsafe_allow_html=True)
        
        col_pitch1, col_pitch2 = st.columns(2)
        with col_pitch1:
            st.markdown("### 🧑‍✈️ Away Starting Pitcher (RHP)")
            st.write("**ERA:** 2.45 | **FIP:** 2.62 | **WHIP:** 1.01 | **K%:** 24.5%")
        with col_pitch2:
            st.markdown("### 🧑‍✈️ Home Starting Pitcher (LHP)")
            st.write("**ERA:** 3.10 | **FIP:** 2.95 | **WHIP:** 1.15 | **K%:** 21.0%")
            
        st.markdown("---")
        st.subheader("📊 Engine Matrix Score (100 Points)")
        
        # Tableau style matrice d'évaluation
        matrix_df = pd.DataFrame({
            "Moteur Analytique": ["Starting Pitching", "Pitch Arsenal Compatibility", "Hitter Evaluation (1-9)", "Bullpen Depth", "Defense & Baserunning", "Park Factor", "Weather / Wind", "Recent Form (Last 15)"],
            "Avantage": ["AWAY", "HOME", "AWAY", "EVEN", "AWAY", "NEUTRAL", "DOME", "AWAY"],
            "Score Comparé (Away / Home)": ["11.2 / 9.8", "8.5 / 9.0", "8.0 / 6.5", "4.0 / 4.0", "3.5 / 2.0", "1.5 / 1.5", "2.0 / 1.0", "6.5 / 3.5"],
            "Validation": ["✅ Pass", "✅ Pass", "✅ Pass", "✅ Pass", "✅ Pass", "✅ Pass", "✅ Pass", "✅ Pass"]
        })
        st.dataframe(matrix_df, use_container_width=True)
        
        col_prop1, col_prop2 = st.columns(2)
        with col_prop1:
            st.markdown("### 🎯 Home Run Targets")
            st.markdown("1. **Batter A** (ISO .280 vs LHP)\n2. **Batter B** (Launch Angle Optimal)")
        with col_prop2:
            st.markdown("### ⚾ Strikeout Targets")
            st.markdown("1. **Away Starting Pitcher** : **OVER 6.5 K's**")
            
        st.markdown("---")
        st.markdown("### 💡 Final Word & Projections")
        st.success("Projeté Total : **7.3 Runs** (Under Lean). Le modèle accorde un avantage tactique net aux visiteurs grâce à la supériorité du FIP du lanceur partant.")
...
with tab3:
    ...
    with st.form("clv_form"):
        ...
        bet_input = c2.text_input("Pari (ex: Away ML)")
